...
HWND EditCreate(HWND hwndParent) {
	HWND hwnd;
	DWORD pDoc;
	hwnd = CreateWindowEx(NULL,//WS_EX_CLIENTEDGE,
						  L"Scintilla",
						  NULL,
						  ...
						  NULL);
	hwndEditBR = hwnd;
	hwndEditTL = CreateWindowEx(NULL,//WS_EX_CLIENTEDGE,
								L"Scintilla",
								NULL,
								WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS,
								0, 0, 0, 0,
	...
	hwndEditTR = CreateWindowEx(NULL,//WS_EX_CLIENTEDGE,
								L"Scintilla",
								NULL,
								WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS,
								0, 0, 0, 0,
								...
								g_hInstance,
								NULL);
	hwndEditBL = CreateWindowEx(NULL,//WS_EX_CLIENTEDGE,
								L"Scintilla",
								NULL,
								WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS,
								0, 0, 0, 0,
								...
								g_hInstance,
								NULL);
	pDoc = SendMessage(hwndEditBR, SCI_GETDOCPOINTER, 0, 0);
	SendMessage(hwndEditTL, SCI_SETDOCPOINTER, 0, pDoc);
	SendMessage(hwndEditTR, SCI_SETDOCPOINTER, 0, pDoc);
	SendMessage(hwndEditBL, SCI_SETDOCPOINTER, 0, pDoc);

	SendEditMsg(SCI_SETCODEPAGE, iDefaultCodePage, 0);
	SendEditMsg(SCI_SETEOLMODE, SC_EOL_CRLF, 0);
	SendEditMsg(SCI_SETPASTECONVERTENDINGS, 1, 0);
	SendEditMsg(SCI_SETMODEVENTMASK, /*SC_MODEVENTMASKALL*/SC_MOD_INSERTTEXT | SC_MOD_DELETETEXT, 0);
	SendEditMsg(SCI_USEPOPUP, FALSE, 0);
	SendEditMsg(SCI_SETSCROLLWIDTH, 2048, 0);
	SendEditMsg(SCI_SETSCROLLWIDTHTRACKING, TRUE, 0);
	SendEditMsg(SCI_SETENDATLASTLINE, TRUE, 0);
	SendEditMsg(SCI_SETCARETSTICKY, SC_CARETSTICKY_OFF, 0);
	SendEditMsg(SCI_SETXCARETPOLICY, CARET_SLOP | CARET_EVEN, 50);
	SendEditMsg(SCI_SETYCARETPOLICY, CARET_EVEN, 0);
	SendEditMsg(SCI_SETMULTIPLESELECTION, FALSE, 0);
	SendEditMsg(SCI_SETADDITIONALSELECTIONTYPING, FALSE, 0);
	SendEditMsg(SCI_SETVIRTUALSPACEOPTIONS, SCVS_NONE, 0);
	SendEditMsg(SCI_SETADDITIONALCARETSBLINK, FALSE, 0);
	SendEditMsg(SCI_SETADDITIONALCARETSVISIBLE, FALSE, 0);
	// style both before and after the visible text in the background
	SendEditMsg(SCI_SETIDLESTYLING, SC_IDLESTYLING_ALL, 0);

	SendEditMsg(SCI_ASSIGNCMDKEY, (SCK_NEXT + (SCMOD_CTRL << 16)), SCI_PARADOWN);
	SendEditMsg(SCI_ASSIGNCMDKEY, (SCK_PRIOR + (SCMOD_CTRL << 16)), SCI_PARAUP);
	SendEditMsg(SCI_ASSIGNCMDKEY, (SCK_NEXT + ((SCMOD_CTRL | SCMOD_SHIFT) << 16)), SCI_PARADOWNEXTEND);
	SendEditMsg(SCI_ASSIGNCMDKEY, (SCK_PRIOR + ((SCMOD_CTRL | SCMOD_SHIFT) << 16)), SCI_PARAUPEXTEND);
	SendEditMsg(SCI_ASSIGNCMDKEY, (SCK_HOME + (0 << 16)), SCI_VCHOMEWRAP);
	SendEditMsg(SCI_ASSIGNCMDKEY, (SCK_END + (0 << 16)), SCI_LINEENDWRAP);
	SendEditMsg(SCI_ASSIGNCMDKEY, (SCK_HOME + (SCMOD_SHIFT << 16)), SCI_VCHOMEWRAPEXTEND);
	SendEditMsg(SCI_ASSIGNCMDKEY, (SCK_END + (SCMOD_SHIFT << 16)), SCI_LINEENDWRAPEXTEND);
...
extern int SendEditMsg(int msg, int param1, int param2) {
	SendMessage(hwndEditTL, msg, param1, param2);
	SendMessage(hwndEditTR, msg, param1, param2);
	SendMessage(hwndEditBL, msg, param1, param2);
	return SendMessage(hwndEditBR, msg, param1, param2);
}
//=============================================================================
//
// EditSetNewText()
...
void EditJoinLinesEx(HWND hwnd) {
	...
	int	 cchEOL = 2;
	BOOL bModified = FALSE;
	//MessageBox(hwnd, L"LINE JOIN 0", L"test", MB_OK);
	if (SC_SEL_RECTANGLE == SendMessage(hwnd, SCI_GETSELECTIONMODE, 0, 0)) {
		MsgBox(MBWARN, IDS_SELRECT);
		return;
	}
	//MessageBox(hwnd, L"LINE JOIN 0-1", L"test", MB_OK);
	iCurPos		 = (int)SendMessage(hwnd, SCI_GETCURRENTPOS, 0, 0);
	iAnchorPos = (int)SendMessage(hwnd, SCI_GETANCHOR, 0, 0);
	//MessageBox(hwnd, L"LINE JOIN 0-2", L"test", MB_OK);
	if (iCurPos == iAnchorPos) {
		return;
	} else {
		iSelStart = (int)SendMessage(hwnd, SCI_GETSELECTIONSTART, 0, 0);
	...
	///MessageBox(hwnd, L"LINE JOIN 0-3", L"test", MB_OK);
	iLine = (int)SendMessage(hwnd, SCI_LINEFROMPOSITION, (WPARAM)iSelStart, 0);
	iSelStart = (int)SendMessage(hwnd, SCI_POSITIONFROMLINE, (WPARAM)iLine, 0);
	...
	//MessageBox(hwnd, L"LINE JOIN 0-5", L"test", MB_OK);
	...
	tr.lpstrText = pszText;
	SendMessage(hwnd, SCI_GETTEXTRANGE, 0, (LPARAM)&tr);
	//MessageBox(hwnd, L"LINE JOIN 0-6", L"test", MB_OK);
	cEOLMode = (int)SendMessage(hwnd, SCI_GETEOLMODE, 0, 0);
	if (cEOLMode == SC_EOL_CR) {
		cchEOL = 1;
	} else if (cEOLMode == SC_EOL_LF) {
	...
	cchJoin = 0;
	//MessageBox(hwnd, L"LINE JOIN 0-7", L"test", MB_OK);
	...
	//MessageBox(hwnd, L"LINE JOIN 1", L"test", MB_OK);
	LocalFree(pszText);
	//MessageBox(hwnd, L"LINE JOIN 2", L"test", MB_OK);
	if (bModified) {
		if (iAnchorPos > iCurPos) {
			iCurPos = iSelStart;
	...
	//MessageBox(hwnd,L"LINE JOIN",L"test",MB_OK);
	LocalFree(pszJoin);
}
