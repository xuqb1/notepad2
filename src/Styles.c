...
void Style_Load(void) {
	...
	crCustom[16] = RGB(0xCC, 0xCC, 0xCC);
	crCustom[17] = RGB(0xCC, 0xCC, 0xCC);
...
void Style_SetLexer(HWND hwnd, PEDITLEXER pLexNew) {
	...
	// Lexer
	SendEditMsg(SCI_SETLEXER, pLexNew->iLexer, 0);

	_itoa(pLexNew->rid - 63000, msg, 10);
	SendEditMsg(SCI_SETPROPERTY, (WPARAM)"lexer.lang.type", (LPARAM)msg);
	...
	if (IsDocLowerKeywords(pLexNew)) {
		for (int i = 0; i < NUMKEYWORD; i++) {
			...
			if (pKeywords && *pKeywords) {
				...
				lowerKeywords -= iIdx;
				SendEditMsg(SCI_SETKEYWORDS, i, (LPARAM)lowerKeywords);
				LocalFree(lowerKeywords);
			} else {
				SendEditMsg(SCI_SETKEYWORDS, i, (LPARAM)"");
			}
		}
	} else {
		for (int i = 0; i < NUMKEYWORD; i++) {
			const char *pKeywords = pLexNew->pKeyWords->pszKeyWords[i];
			if (pKeywords && *pKeywords) {
				SendEditMsg(SCI_SETKEYWORDS, i, (LPARAM)pKeywords);
			} else {
				SendEditMsg(SCI_SETKEYWORDS, i, (LPARAM)"");
			}
		}
	...
	// Clear
	SendEditMsg(SCI_CLEARDOCUMENTSTYLE, 0, 0);

	// Default Values are always set
	SendEditMsg(SCI_STYLERESETDEFAULT, 0, 0);
	SendEditMsg(SCI_STYLESETCHARACTERSET, STYLE_DEFAULT, (LPARAM)DEFAULT_CHARSET);
	...
	if (!Style_StrGetColor(TRUE, lexDefault.Styles[0 + iIdx].szValue, &iValue)) {
		SendEditMsg(SCI_STYLESETFORE, STYLE_DEFAULT, (LPARAM)GetSysColor(COLOR_WINDOWTEXT));    // default text color
	}
	if (!Style_StrGetColor(FALSE, lexDefault.Styles[0 + iIdx].szValue, &iValue)) {
		SendEditMsg(SCI_STYLESETBACK, STYLE_DEFAULT, (LPARAM)GetSysColor(COLOR_WINDOW));    // default window color
	}
	...
	SendEditMsg(SCI_STYLECLEARALL, 0, 0);
	...
	if (Style_StrGetColor(TRUE, lexDefault.Styles[6 + iIdx].szValue, &rgb)) { // selection fore
		SendEditMsg(SCI_SETSELFORE, TRUE, rgb);
		SendEditMsg(SCI_SETADDITIONALSELFORE, rgb, 0);
	} else {
		SendEditMsg(SCI_SETSELFORE, 0, 0);
		SendEditMsg(SCI_SETADDITIONALSELFORE, 0, 0);
	}

	if (Style_StrGetColor(FALSE, lexDefault.Styles[6 + iIdx].szValue, &iValue)) { // selection back
		SendEditMsg(SCI_SETSELBACK, TRUE, iValue);
		SendEditMsg(SCI_SETADDITIONALSELBACK, iValue, 0);
	} else {
		SendEditMsg(SCI_SETSELBACK, TRUE, RGB(0xC0, 0xC0, 0xC0)); // use a default value...
		SendEditMsg(SCI_SETADDITIONALSELBACK, RGB(0xC0, 0xC0, 0xC0), 0);
	}

	if (Style_StrGetAlpha(lexDefault.Styles[6 + iIdx].szValue, &iValue)) { // selection alpha
		SendEditMsg(SCI_SETSELALPHA, iValue, 0);
		SendEditMsg(SCI_SETADDITIONALSELALPHA, iValue, 0);
	} else {
		SendEditMsg(SCI_SETSELALPHA, SC_ALPHA_NOALPHA, 0);
		SendEditMsg(SCI_SETADDITIONALSELALPHA, SC_ALPHA_NOALPHA, 0);
	}

	if (StrStrI(lexDefault.Styles[6 + iIdx].szValue, L"eolfilled")) { // selection eolfilled
		SendEditMsg(SCI_SETSELEOLFILLED, 1, 0);
	} else {
		SendEditMsg(SCI_SETSELEOLFILLED, 0, 0);
	}

	if (Style_StrGetColor(TRUE, lexDefault.Styles[7 + iIdx].szValue, &rgb)) { // whitespace fore
		SendEditMsg(SCI_SETWHITESPACEFORE, TRUE, rgb);
	} else {
		SendEditMsg(SCI_SETWHITESPACEFORE, 0, 0);
	}

	if (Style_StrGetColor(FALSE, lexDefault.Styles[7 + iIdx].szValue, &rgb)) { // whitespace back
		SendEditMsg(SCI_SETWHITESPACEBACK, TRUE, rgb);
	} else {
		SendEditMsg(SCI_SETWHITESPACEBACK, 0, 0);    // use a default value...
	}
	...
	SendEditMsg(SCI_SETWHITESPACESIZE, iValue, 0);

	if (bHiliteCurrentLine) {
		if (Style_StrGetColor(FALSE, lexDefault.Styles[8 + iIdx].szValue, &rgb)) { // caret line back
			SendEditMsg(SCI_SETCARETLINEVISIBLE, TRUE, 0);
			SendEditMsg(SCI_SETCARETLINEBACK, rgb, 0);

			if (Style_StrGetAlpha(lexDefault.Styles[8 + iIdx].szValue, &iValue)) {
				SendEditMsg(SCI_SETCARETLINEBACKALPHA, iValue, 0);
			} else {
				SendEditMsg(SCI_SETCARETLINEBACKALPHA, SC_ALPHA_NOALPHA, 0);
			}
		} else {
			SendEditMsg(SCI_SETCARETLINEVISIBLE, FALSE, 0);
		}
	} else {
		SendEditMsg(SCI_SETCARETLINEVISIBLE, FALSE, 0);
	}
	...
	if (StrStr(lexDefault.Styles[9 + iIdx].szValue, L"block")) {
		SendEditMsg(SCI_SETCARETSTYLE, CARETSTYLE_BLOCK, 0);
		...
		SendEditMsg(SCI_SETCARETSTYLE, CARETSTYLE_LINE, 0);
		SendEditMsg(SCI_SETCARETWIDTH, iValue, 0);
	}

	if (StrStr(lexDefault.Styles[9 + iIdx].szValue, L"noblink")) {
		SendEditMsg(SCI_SETCARETPERIOD, (WPARAM)0, 0);
		if (lstrlen(wchCaretStyle)) {
			lstrcat(wchCaretStyle, L"; ");
		}
		lstrcat(wchCaretStyle, L"noblink");
	} else {
		SendEditMsg(SCI_SETCARETPERIOD, (WPARAM)GetCaretBlinkTime(), 0);
	}
	...
	SendEditMsg(SCI_SETCARETFORE, rgb, 0);
	SendEditMsg(SCI_SETADDITIONALCARETFORE, rgb, 0);
	...
	if (SendMessage(hwnd, SCI_GETEDGEMODE, 0, 0) == EDGE_LINE) {
		if (Style_StrGetColor(TRUE, lexDefault.Styles[10 + iIdx].szValue, &rgb)) { // edge fore
			SendEditMsg(SCI_SETEDGECOLOUR, rgb, 0);
		} else {
			SendEditMsg(SCI_SETEDGECOLOUR, GetSysColor(COLOR_3DLIGHT), 0);
		}
	} else {
	...
	if (Style_StrGetSize(lexDefault.Styles[11 + iIdx].szValue, &iValue) && pLexNew != &lexANSI) {
		...
		iAscent += iValue / 2;
		iDescent += iValue / 2;
		SendEditMsg(SCI_SETEXTRAASCENT, (WPARAM)iAscent, 0);
		SendEditMsg(SCI_SETEXTRADESCENT, (WPARAM)iDescent, 0);
	} else {
		SendEditMsg(SCI_SETEXTRAASCENT, 0, 0);
		SendEditMsg(SCI_SETEXTRADESCENT, 0, 0);
	...
	// set folding style; braces are for scoping only
	{
		...
		SendEditMsg(SCI_SETFOLDMARGINCOLOUR, TRUE, crCustom[16]); //clrBack);
		//SciCall_SetFoldMarginHiColour(TRUE, clrBack);
		SendEditMsg(SCI_SETFOLDMARGINHICOLOUR, TRUE, crCustom[17]);// clrBack);
		...
		for (unsigned int i = 0; i < COUNTOF(iMarkerIDs); ++i) {
			...
			SendEditMsg(SCI_MARKERSETBACK,iMarkerIDs[i], clrFore);
			//SciCall_MarkerSetFore(iMarkerIDs[i], clrBack);
			SendEditMsg(SCI_MARKERSETFORE,iMarkerIDs[i], clrBack);
		}
	...
	SendEditMsg(SCI_COLOURISE, 0, (LPARAM) - 1);
...
void Style_SetIndentGuides(HWND hwnd, BOOL bShow) {
	...
	SendEditMsg(SCI_SETINDENTATIONGUIDES, iIndentView, 0);
}
...
void Style_SetStyles(HWND hwnd, int iStyle, LPCWSTR lpszStyle) {
	...
	if (Style_StrGetFont(lpszStyle, tch, COUNTOF(tch))) {
		...
		SendEditMsg(SCI_STYLESETFONT, iStyle, (LPARAM)mch);
	}

	// Size
	if (Style_StrGetSize(lpszStyle, &iValue)) {
		iValue = MulDiv(iValue, g_uCurrentDPI, USER_DEFAULT_SCREEN_DPI);
		SendEditMsg(SCI_STYLESETSIZE, iStyle, (LPARAM)iValue);
	}

	// Fore
	if (Style_StrGetColor(TRUE, lpszStyle, &iValue)) {
		SendEditMsg(SCI_STYLESETFORE, iStyle, (LPARAM)iValue);
	}

	// Back
	if (Style_StrGetColor(FALSE, lpszStyle, &iValue)) {
		SendEditMsg(SCI_STYLESETBACK, iStyle, (LPARAM)iValue);
	}

	// Bold
	if (StrStrI(lpszStyle, L"bold")) {
		SendEditMsg(SCI_STYLESETBOLD, iStyle, (LPARAM)TRUE);
	} else {
		SendEditMsg(SCI_STYLESETBOLD, iStyle, (LPARAM)FALSE);
	}

	// Italic
	if (StrStrI(lpszStyle, L"italic")) {
		SendEditMsg(SCI_STYLESETITALIC, iStyle, (LPARAM)TRUE);
	} else {
		SendEditMsg(SCI_STYLESETITALIC, iStyle, (LPARAM)FALSE);
	}

	// Underline
	if (StrStrI(lpszStyle, L"underline")) {
		SendEditMsg(SCI_STYLESETUNDERLINE, iStyle, (LPARAM)TRUE);
	} else {
		SendEditMsg(SCI_STYLESETUNDERLINE, iStyle, (LPARAM)FALSE);
	}

	// Strike
	if (StrStrI(lpszStyle, L"strike")) {
		SendEditMsg(SCI_STYLESETSTRIKE, iStyle, (LPARAM)TRUE);
	} else {
		SendEditMsg(SCI_STYLESETSTRIKE, iStyle, (LPARAM)FALSE);
	}

	// EOL Filled
	if (StrStrI(lpszStyle, L"eolfilled")) {
		SendEditMsg(SCI_STYLESETEOLFILLED, iStyle, (LPARAM)TRUE);
	} else {
		SendEditMsg(SCI_STYLESETEOLFILLED, iStyle, (LPARAM)FALSE);
	}

	// Case
	if (Style_StrGetCase(lpszStyle, &iValue)) {
		SendEditMsg(SCI_STYLESETCASE, iStyle, (LPARAM)iValue);
	}

	// Character Set
	if (Style_StrGetCharSet(lpszStyle, &iValue)) {
		SendEditMsg(SCI_STYLESETCHARACTERSET, iStyle, (LPARAM)iValue);
	}
...
void Style_SetFontQuality(HWND hwnd, LPCWSTR lpszStyle) {
	...
	SendEditMsg(SCI_SETFONTQUALITY, wQuality, 0);
}
