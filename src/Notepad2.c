...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpCmdLine, int nCmdShow) {
	...
	hHSplit = LoadCursor(g_hInstance,IDC_HSPLIT);
	hVSplit = LoadCursor(g_hInstance, IDC_VSPLIT);
...
HWND InitInstance(HINSTANCE hInstance, LPSTR pszCmdLine, int nCmdShow) {
	...
	if (lpFileArg /*&& !flagNewFromClipboard*/) {
		...
		if (iSrcEncoding != -1) {
			...
			SendEditMsg(SCI_SETCODEPAGE, (iEncoding == CPI_DEFAULT) ? iDefaultCodePage : SC_CP_UTF8, 0);
		}
	}
	...
	if (flagNewFromClipboard) {
		if (SendMessage(hwndEdit, SCI_CANPASTE, 0, 0)) {
			...
			SendEditMsg(SCI_BEGINUNDOACTION, 0, 0);
			if (SendMessage(hwndEdit, SCI_GETLENGTH, 0, 0) > 0) {
				SendEditMsg(SCI_NEWLINE, 0, 0);
			}
			SendEditMsg(SCI_PASTE, 0, 0);
			SendEditMsg(SCI_NEWLINE, 0, 0);
			SendEditMsg(SCI_ENDUNDOACTION, 0, 0);
	...
	if (flagMatchText && lpMatchArg) {
		if (lstrlen(lpMatchArg) && SendMessage(hwndEdit, SCI_GETLENGTH, 0, 0)) {
			...
			if (flagMatchText & 2) {
				...
				if (!flagJumpTo) {
					SendEditMsg(SCI_DOCUMENTSTART, 0, 0);
				}
...
LRESULT CALLBACK MainWndProc(HWND hwnd, UINT umsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_SIZE:
		...
		if (bShowToolbar) {
			...
			//SendMessage(hwndReBar,umsg,lParam,wParam);
		}
		if (bShowStatusbar) {
			SendMessage(hwndStatus,umsg,lParam,wParam);
		}
		if (wi.splity < cyReBar+cyReBarFrame) {
			wi.splity = cyReBar + cyReBarFrame;
		}
		if (wi.oldx < 0) {
			wi.oldx = 0;
		}
		...
		if (wi.splity > cyReBar + cyReBarFrame) {
			wi.splity = MulDiv(HIWORD(lParam), wi.splity, wi.oldy);
			if (wi.splitxt > 0) {
				wi.splitxt = MulDiv(LOWORD(lParam), wi.splitxt, wi.oldx);
			}
		}
		if (wi.splitxb > 0) {
			wi.splitxb = MulDiv(LOWORD(lParam),wi.splitxb,wi.oldx);
		}
		if (wi.splitxt < 0) {
			wi.splitxt = 0;
		}
		...
		wi.oldy = HIWORD(lParam);
		wi.oldx = LOWORD(lParam);
		SetRect(&staticrc, 0, cyReBar + cyReBarFrame, wi.oldx, wi.oldy - (cyReBar+cyReBarFrame));
		...
		break;
	case WM_MOUSEMOVE:
		toolbarHeight = cyReBar + cyReBarFrame;
		GetWindowRect(hwndStatus, &rc);
		statusbarHeight = rc.bottom - rc.top;
		pt.x = LOWORD(lParam);
		pt.y = HIWORD(lParam);
		//MessageBox(hwnd, L"test1", L"test", MB_OK);
		if (GetCapture() == hwnd) {
			ClipCursor(&rc2);
			if (Ysplit == 1) {
				wi.splity = pt.y;
				if (pt.y < staticrc.bottom - SPLITSIZE && (int)pt.y >= toolbarHeight) {
				}
				else {
					wi.splity = toolbarHeight;
					ReleaseCapture();
					ClipCursor(0);
					SetCursor(0);
					SetCursor(hArrow);
				...
				if (pt.x < staticrc.right - SPLITSIZE && (int)pt.x >= 0) {

				}
				else {
					if (pt.y > wi.splity + SPLITSIZE) {
						wi.splitxb = 0;
					}
					else {
						wi.splitxt = 0;
			...
			GetClientRect(hwnd, &staticrc);
			staticrc.top = toolbarHeight;
			staticrc.bottom = staticrc.bottom - statusbarHeight;
			SetRect(&rc2, staticrc.left, wi.splity, staticrc.right, wi.splity + SPLITSIZE);
			if (PtInRect(&rc2, pt)) {
				SetCursor(hHSplit);
				Ysplit = 1;
				break;
			}
			SetRect(&rc2, wi.splitxt, toolbarHeight, wi.splitxt + SPLITSIZE, wi.splity);
			if (PtInRect(&rc2, pt)) {
				SetCursor(hVSplit);
				Ysplit = 0;
				break;
			}
			SetRect(&rc2, wi.splitxb, wi.splity + SPLITSIZE, wi.splitxb + SPLITSIZE, staticrc.bottom);
			if (PtInRect(&rc2, pt)) {
				SetCursor(hVSplit);
				Ysplit = 0;
				break;
	...
	case WM_LBUTTONDOWN:
		if (Ysplit >= 0) {
			...
			if (Ysplit == 1) {
				pt.x = LOWORD(lParam);
				pt.y = wi.splity;
				SetRect(&rc2, pt.x, -1, pt.x + 1, staticrc.bottom);
				SetCursor(hHSplit);
			}
			else if (Ysplit == 0) {
				pt.y = HIWORD(lParam);
				pt.x = (pt.y > wi.splity) ? wi.splitxb : wi.splitxt;
				SetRect(&rc2, -1, pt.y, staticrc.right, pt.y + 1);
				SetCursor(hVSplit);
			}
			MapWindowPoints(hwnd, 0, &rc2, 2);
	...
	case WM_CONTEXTMENU: {
		...
		if ((nID != IDC_EDIT) && (nID != IDC_EDITBL) && (nID != IDC_EDITTL) && (nID != IDC_EDITTR) && (nID != IDC_STATUSBAR) &&
		...
		case IDC_EDIT: {
			...
			//MessageBox(hwnd,L"test BR",L"test",MB_OK);
		...
		break;
		case IDC_EDITTL: { //,IDC_EDITTL,IDC_EDITTR,IDC_EDITBL
			hwndEdit = hwndEditTL;
			InitScintillaHandle(hwndEdit);
			int iSelStart	= (int)SendMessage(hwndEdit, SCI_GETSELECTIONSTART, 0, 0);
			int iSelEnd		= (int)SendMessage(hwndEdit, SCI_GETSELECTIONEND, 0, 0);
		...
		break;
		case IDC_EDITTR: { //,IDC_EDITTL,IDC_EDITTR,IDC_EDITBL
			hwndEdit = hwndEditTR;
			InitScintillaHandle(hwndEdit);
		...
		break;
		case IDC_EDITBL: { //,IDC_EDITTL,IDC_EDITTR,IDC_EDITBL
			hwndEdit = hwndEditBL;
			InitScintillaHandle(hwndEdit);
			int iSelStart	= (int)SendMessage(hwndEdit, SCI_GETSELECTIONSTART, 0, 0);
			int iSelEnd		= (int)SendMessage(hwndEdit, SCI_GETSELECTIONEND, 0, 0);
...
void AssignFocus() {
	HWND dwFocus;
	dwFocus = GetFocus();
	if (wi.splity <= 0 && (dwFocus == hwndEditTR || dwFocus == hwndEditTL)) {
		hwndEdit = hwndEditBR;
		SetFocus(hwndEdit);
	}
	else {
		if (wi.splitxt <= 0 && dwFocus == hwndEditTL) {
			hwndEdit = hwndEditTR;
			SetFocus(hwndEdit);
		}
		else {
			if (wi.splitxb <= 0 && dwFocus == hwndEditBL) {
				hwndEdit = hwndEditBR;
				SetFocus(hwndEdit);
			}
		}
...
static inline void UpdateSelectionMarginWidth() {
	...
	SendEditMsg(SCI_SETMARGINWIDTHN, 1, width);
}
...
static inline void UpdateFoldMarginWidth() {
	...
	SendEditMsg(SCI_SETMARGINWIDTHN, MARGIN_FOLD_INDEX, width);
}

#define SetCallTipsWaitTime() \
	SendEditMsg(SCI_SETMOUSEDWELLTIME, bShowCallTips? iCallTipsWaitTime : SC_TIME_FOREVER, 0);
...
LRESULT MsgCreate(HWND hwnd, WPARAM wParam, LPARAM lParam) {
	...
	InitScintillaHandle(hwndEdit);
	//这里需要给不同的g_hScintilla赋值，待完成，需要仔细考虑，以解决代码卷展问题
	//InitScintillaHandle(hwndEditTL);
	//InitScintillaHandle(hwndEditTR);
	//InitScintillaHandle(hwndEditBL);
	//InitScintillaHandle(hwndEditBR);
	// Tabs
	SendEditMsg(SCI_SETUSETABS, !bTabsAsSpaces, 0);
	SendEditMsg(SCI_SETTABINDENTS, bTabIndents, 0);
	SendEditMsg(SCI_SETBACKSPACEUNINDENTS, bBackspaceUnindents, 0);
	SendEditMsg(SCI_SETTABWIDTH, iTabWidth, 0);
	SendEditMsg(SCI_SETINDENT, iIndentWidth, 0);

	// Indent Guides
	Style_SetIndentGuides(hwndEditTL, bShowIndentGuides);
	Style_SetIndentGuides(hwndEditTR, bShowIndentGuides);
	Style_SetIndentGuides(hwndEditBL, bShowIndentGuides);
	Style_SetIndentGuides(hwndEditBR, bShowIndentGuides);
	// Word wrap
	if (!fWordWrap) {
		SendEditMsg(SCI_SETWRAPMODE, SC_WRAP_NONE, 0);
	} else {
		SendEditMsg(SCI_SETWRAPMODE, (iWordWrapMode == 0) ? SC_WRAP_WORD : SC_WRAP_CHAR, 0);
	}
	if (iWordWrapIndent == 5) {
		SendEditMsg(SCI_SETWRAPINDENTMODE, SC_WRAPINDENT_SAME, 0);
	} else if (iWordWrapIndent == 6) {
		SendEditMsg(SCI_SETWRAPINDENTMODE, SC_WRAPINDENT_INDENT, 0);
	} else {
		int i = 0;
		...
		SendEditMsg(SCI_SETWRAPSTARTINDENT, i, 0);
		SendEditMsg(SCI_SETWRAPINDENTMODE, SC_WRAPINDENT_FIXED, 0);
	}
	...
	if (bShowWordWrapSymbols) {
		...
		SendEditMsg(SCI_SETWRAPVISUALFLAGSLOCATION, wrapVisualFlagsLocation, 0);
		SendEditMsg(SCI_SETWRAPVISUALFLAGS, wrapVisualFlags, 0);
	} else {
		SendEditMsg(SCI_SETWRAPVISUALFLAGS, 0, 0);
	}

	// Long Lines
	if (bMarkLongLines) {
		SendEditMsg(SCI_SETEDGEMODE, (iLongLineMode == EDGE_LINE) ? EDGE_LINE : EDGE_BACKGROUND, 0);
	} else {
		SendEditMsg(SCI_SETEDGEMODE, EDGE_NONE, 0);
	}
	SendEditMsg(SCI_SETEDGECOLUMN, iLongLinesLimit, 0);

	// Margins
	SendEditMsg(SCI_SETMARGINWIDTHN, 2, 0);
	...
	SendEditMsg(SCI_SETMARGINTYPEN,MARGIN_FOLD_INDEX, SC_MARGIN_SYMBOL);
	//SciCall_SetMarginMask(MARGIN_FOLD_INDEX, SC_MASK_FOLDERS);
	SendEditMsg(SCI_SETMARGINMASKN, MARGIN_FOLD_INDEX, SC_MASK_FOLDERS);
	UpdateFoldMarginWidth();
	SendEditMsg(SCI_SETMARGINSENSITIVEN,MARGIN_FOLD_INDEX, TRUE);
	// Code folding, Box tree
	SendEditMsg(SCI_MARKERDEFINE,SC_MARKNUM_FOLDEROPEN, SC_MARK_BOXMINUS);
	SendEditMsg(SCI_MARKERDEFINE, SC_MARKNUM_FOLDER, SC_MARK_BOXPLUS);
	SendEditMsg(SCI_MARKERDEFINE, SC_MARKNUM_FOLDERSUB, SC_MARK_VLINE);
	SendEditMsg(SCI_MARKERDEFINE, SC_MARKNUM_FOLDERTAIL, SC_MARK_LCORNER);
	SendEditMsg(SCI_MARKERDEFINE, SC_MARKNUM_FOLDEREND, SC_MARK_BOXPLUSCONNECTED);
	SendEditMsg(SCI_MARKERDEFINE, SC_MARKNUM_FOLDEROPENMID, SC_MARK_BOXMINUSCONNECTED);
	SendEditMsg(SCI_MARKERDEFINE, SC_MARKNUM_FOLDERMIDTAIL, SC_MARK_TCORNER);
#if !NP2_DEBUG_FOLD_LEVEL
	// Draw below if not expanded
	SendEditMsg(SCI_SETFOLDFLAGS,SC_FOLDFLAG_LINEAFTER_CONTRACTED,0);
#else
	//SciCall_SetFoldFlags(SC_FOLDFLAG_LEVELNUMBERS);
	SendEdit(SCI_SETFOLDFLAGS,SC_FOLDFLAG_LEVELNUMBERS,0);
#endif
	// highlight for current folding block
	//SciCall_MarkerEnableHighlight(TRUE);
	SendEditMsg(SCI_MARKERENABLEHIGHLIGHT, TRUE, 0);
	// CallTips
	SetCallTipsWaitTime();

	// Nonprinting characters
	SendEditMsg(SCI_SETVIEWWS, (bViewWhiteSpace) ? SCWS_VISIBLEALWAYS : SCWS_INVISIBLE, 0);
	SendEditMsg(SCI_SETVIEWEOL, bViewEOLs, 0);
...
void MsgDPIChanged(HWND hwnd, WPARAM wParam, LPARAM lParam) {
	...
	SendEditMsg(SCI_INSERTTEXT, 0, (LPARAM)buf);
#endif
...
void DrawWindow(HWND hwnd, BOOL hMove) {
	RECT rc;
	int w, h;
	int x, y;
	int toolbarHeight = 0;
	...
	GetClientRect(hwnd,&rc);
	rc.top = toolbarHeight;// cyReBar + cyReBarFrame;
	rc.bottom -= statusbarHeight;
	w = rc.right;
	h = rc.bottom;// -rc.top;
	if (wi.splity < toolbarHeight) {
		wi.splity = toolbarHeight;
	}
	if (wi.splitxb < 0) {
		wi.splitxb = 0;
	...
	if (hMove) {
		...
		if (wi.splity > toolbarHeight) {
			hdwp= DeferWindowPos(hdwp, hwndEditTR, NULL, 
				wi.splitxt + SPLITSIZE, toolbarHeight, 
				w - (wi.splitxt + SPLITSIZE), wi.splity - toolbarHeight,
				SWP_NOZORDER | SWP_SHOWWINDOW | 
				SWP_NOACTIVATE | SWP_NOCOPYBITS);
			if (wi.splitxt > 0) {
				hdwp = DeferWindowPos(hdwp,hwndEditTL,NULL,
					0,toolbarHeight,wi.splitxt,wi.splity-toolbarHeight,
					SWP_NOZORDER | SWP_SHOWWINDOW | 
					SWP_NOACTIVATE | SWP_NOCOPYBITS);
			}
			else {
				ShowWindow(hwndEditTL, SW_HIDE);
			}
		}
		else {
			ShowWindow(hwndEditTR, SW_HIDE);
			ShowWindow(hwndEditTL, SW_HIDE);
		}
		if (wi.splity < h - SPLITSIZE) {
			//MessageBox(hwnd,L"test",L"promt",MB_OK);
			hdwp = DeferWindowPos(hdwp, hwndEditBR, NULL,
				wi.splitxb + SPLITSIZE, wi.splity + SPLITSIZE,
				w-(wi.splitxb+SPLITSIZE),h-wi.splity-SPLITSIZE,
				SWP_NOZORDER | SWP_SHOWWINDOW | 
				SWP_NOACTIVATE | SWP_NOCOPYBITS);
			if (wi.splitxb > 0) {
				hdwp = DeferWindowPos(hdwp, hwndEditBL, NULL,
					0,wi.splity+SPLITSIZE,wi.splitxb,h-wi.splity-SPLITSIZE,
					SWP_NOZORDER | SWP_SHOWWINDOW | 
					SWP_NOACTIVATE | SWP_NOCOPYBITS);
			}
			else {
				ShowWindow(hwndEditBL, SW_HIDE);
			}
		}
		else {
			ShowWindow(hwndEditBR, SW_HIDE);
		}
	...
	HDC hdc = GetDC(hwnd);
	int edge = EDGE_BUMP;//EDGE_SUNKEN EDGE_RAISED EDGE_BUMP EDGE_ETCHED  BDR_RAISEDINNER BDR_SUNKENOUTER BDR_SUNKENINNER | BDR_SUNKENOUTER
	SetRect(&rc, 0, wi.splity, w, wi.splity + SPLITSIZE);
	DrawEdge(hdc, &rc, edge,  BF_BOTTOMRIGHT);
	SetRect(&rc, wi.splitxb, wi.splity + SPLITSIZE, wi.splitxb + SPLITSIZE, h);
	DrawEdge(hdc, &rc, edge, BF_BOTTOMRIGHT);
	SetRect(&rc,wi.splitxt, toolbarHeight, wi.splitxt + SPLITSIZE, wi.splity);
	DrawEdge(hdc, &rc, edge, BF_BOTTOMRIGHT);
	ReleaseDC(hwnd, hdc);
}
...
void MsgSize(HWND hwnd, WPARAM wParam, LPARAM lParam) {
	...
	DeferWindowPos(hdwp, hwndEdit, NULL, wi.splitxb+SPLITSIZE, y+SPLITSIZE, cx, cy-SPLITSIZE, SWP_NOZORDER | SWP_NOACTIVATE);// x + cxEditFrame, y + cyEditFrame,
...
void MsgInitMenu(HWND hwnd, WPARAM wParam, LPARAM lParam) {
	...
	EnableCmd(hmenu, IDM_EDIT_SORTLINES,
			  SendEditMsg(SCI_LINEFROMPOSITION,
...
LRESULT MsgCommand(HWND hwnd, WPARAM wParam, LPARAM lParam) {
	...
	case IDC_EDIT:
		hwndEdit = hwndEditBR;
		//InitScintillaHandle(hwndEditBR);
		//MessageBox(hwnd,L"command BR",L"test",MB_OK);
		break;
	case IDC_EDITTL:
		hwndEdit = hwndEditTL;
		//InitScintillaHandle(hwndEditTL);
		//MessageBox(hwnd, L"command TL", L"test", MB_OK);
		break;
	case IDC_EDITTR:
		//MessageBox(hwnd, L"command TR", L"test", MB_OK);
		hwndEdit = hwndEditTR;
		//InitScintillaHandle(hwndEditTR);
		break;
	case IDC_EDITBL:
		//MessageBox(hwnd, L"command BL", L"test", MB_OK);
		hwndEdit = hwndEditBL;
		//InitScintillaHandle(hwndEditBL);
		break;
	case IDM_FILE_NEW:
	...
	case IDM_EDIT_JOINLINES:
		...
		//MessageBox(hwnd,L"test 1",L"test",MB_OK);
	...
	case IDM_EDIT_INSERT_LONGDATE: {
		...
		if (IniGetString(L"Settings2",
			...
			sst.tm_milliseconds = (int)st.wMilliseconds;
			mktime(&sst);
...
//http://www.frenk.com/2009/12/convert-filetime-to-unix-timestamp/
	...
	case BME_EDIT_BOOKMARKTOGGLE: {
		...
		if (bitmask & 1) {
			// unset
			SendEditMsg(SCI_MARKERDELETE, iLine, 0);
		} else {
			if (bShowSelectionMargin) {
				SendEditMsg(SCI_MARKERDEFINEPIXMAP, 0, (LPARAM)bookmark_pixmap);
			} else {
				SendEditMsg(SCI_MARKERSETBACK, 0, NP2_BookmarkLineForeColor);
				SendEditMsg( SCI_MARKERSETALPHA, 0, NP2_BookmarkLineColorAlpha);
				SendEditMsg(SCI_MARKERDEFINE, 0, SC_MARK_BACKGROUND);
			}

			// set
			SendEditMsg(SCI_MARKERADD, iLine, 0);
		}
	}
	break;
	...
	case BME_EDIT_BOOKMARKCLEAR:
		SendEditMsg(SCI_MARKERDELETEALL, (WPARAM) - 1, 0);
		break;
#endif
	...
	case IDM_VIEW_MATCHBRACES:
		...
		//hwndEdit = GetFocus();
		int editid = IDC_EDIT;
		/*if (hwndEdit == hwndEditBL) {
			editid = IDC_EDITBL;
		}
		if (hwndEdit == hwndEditTR) {
			editid = IDC_EDITTR;
		}
		if (hwndEdit == hwndEditTL) {
			editid = IDC_EDITTL;
		}*/
		if (bMatchBraces) {			
			struct SCNotification scn;			
			scn.nmhdr.hwndFrom = hwndEdit;
			scn.nmhdr.idFrom = editid;//IDC_EDIT ;
			scn.nmhdr.code = SCN_UPDATEUI;
			scn.updated = SC_UPDATE_CONTENT;
			SendMessage(hwnd, WM_NOTIFY, editid, (LPARAM)&scn);
		} else {
	...
	case IDM_VIEW_ZOOMIN:
		//MessageBox(hwnd,L"zoomin",L"test",MB_OK);
		//hwndEdit = GetFocus();
...
LRESULT MsgNotify(HWND hwnd, WPARAM wParam, LPARAM lParam) {
	...
	case IDC_EDITBL:
		...
		switch (pnmh->code) {
		//case NM_CLICK:
		//	//MessageBox(hwnd,L"test click",L"test",MB_OK);
		//	if (SendMessage(hwndEdit, SCI_GETSELECTIONS, 0, 0) > 1 &&
		//		SendMessage(hwndEdit, SCI_GETSELECTIONMODE, 0, 0) != SC_SEL_RECTANGLE) {
		//		int iCurPos = (int)SendMessage(hwndEdit, SCI_GETCURRENTPOS, 0, 0);
		//		SendMessage(hwndEdit, WM_CANCELMODE, 0, 0);
	...
	case IDC_STATUSBAR:
		...
		case NM_CLICK: {
			...
			case STATUS_EOLMODE:
				SendEditMsg(SCI_CONVERTEOLS, SendMessage(hwndEdit, SCI_GETEOLMODE, 0, 0), 0);
				EditFixPositions(hwndEdit);
				return TRUE;
	...
	/*if (pnmh->idFrom == IDC_EDIT || pnmh->idFrom == IDC_EDITTL || pnmh->idFrom == IDC_EDITTR || pnmh->idFrom == IDC_EDITBL) {
		case NM_DBLCLK: {
		hwndEdit = GetFocus();
		EditMarkAll(hwndEdit, iMarkOccurrences, bMarkOccurrencesMatchCase, bMarkOccurrencesMatchWords);
		UpdateStatusbar();
...
void UpdateStatusbar1(HWND hwnd) {
	int iPos;
	int iLn;
	int iLines;
	int iCol;
	...
	iLn = (int)SendMessage(hwnd, SCI_LINEFROMPOSITION, iPos, 0) + 1;
	wsprintf(tchLn, L"%i", iLn);
	FormatNumberStr(tchLn);

	iLines = (int)SendMessage(hwnd, SCI_GETLINECOUNT, 0, 0);
	wsprintf(tchLines, L"%i", iLines);
	FormatNumberStr(tchLines);

	iCol = (int)SendMessage(hwnd, SCI_GETCOLUMN, iPos, 0) + 1;
	wsprintf(tchCol, L"%i", iCol);
	FormatNumberStr(tchCol);

	iLineStart = (int)SendMessage(hwnd, SCI_POSITIONFROMLINE, iLn - 1, 0);
	iLineEnd = (int)SendMessage(hwnd, SCI_GETLINEENDPOSITION, iLn - 1, 0);
	iPos = (int)SendMessage(hwnd, SCI_COUNTCHARACTERS, iLineStart, iPos) + 1;
	iCol = (int)SendMessage(hwnd, SCI_COUNTCHARACTERS, iLineStart, iLineEnd);
	wsprintf(tchCh, L"%i", iPos);
	wsprintf(tchChs, L"%i", iCol);
	FormatNumberStr(tchCh);
	FormatNumberStr(tchChs);

	iCol = (int)SendMessage(hwnd, SCI_GETCOLUMN, iLineEnd, 0);
	wsprintf(tchCols, L"%i", iCol);
	FormatNumberStr(tchCols);
	...
	if (SC_SEL_RECTANGLE != SendMessage(hwnd, SCI_GETSELECTIONMODE, 0, 0)) {
		int iSel = (int)SendMessage(hwnd, SCI_GETSELTEXT, 0, 0) - 1;
		wsprintf(tchSel, L"%i", iSel);
		FormatNumberStr(tchSel);
		iSel = (int)SendMessage(hwnd, SCI_COUNTCHARACTERS, iSelStart, iSelEnd);
		wsprintf(tchSelCh, L"%i", iSel);
		FormatNumberStr(tchSelCh);
	}
	else {
	...
	iStartOfLinePos = (int)SendMessage(hwnd, SCI_POSITIONFROMLINE, iLineEnd, 0);
	iLinesSelected = iLineEnd - iLineStart;
	if (iSelStart != iSelEnd && iStartOfLinePos != iSelEnd) {
		iLinesSelected += 1;
	}
	...
	iBytes = (int)SendMessage(hwnd, SCI_GETLENGTH, 0, 0);
	StrFormatByteSize(iBytes, tchBytes, COUNTOF(tchBytes));
...
#if !NP2_DEBUG_FOLD_LEVEL
			SendEditMsg( SCI_SETMARGINWIDTHN, 0, iLineMarginWidthFit);
#else
			SendEditMsg(SCI_SETMARGINWIDTHN, 0, 100);
#endif
		}
	} else {
		SendEditMsg( SCI_SETMARGINWIDTHN, 0, 0);
	}
}
...
BOOL FileLoad(BOOL bDontSave, BOOL bNew, BOOL bReload, BOOL bNoEncDetect, LPCWSTR lpszFile) {
	...
	if (bNew) {
		...
		SendEditMsg(SCI_SETEOLMODE, iLineEndings[iDefaultEOLMode], 0);
		iEncoding = iDefaultEncoding;
		iOriginalEncoding = iDefaultEncoding;
		SendEditMsg(SCI_SETCODEPAGE, (iDefaultEncoding == CPI_DEFAULT) ? iDefaultCodePage : SC_CP_UTF8, 0);
		//EditSetNewText(hwndEdit, "", 0);
		SetWindowTitle(hwndMain, uidsAppTitle, fIsElevated, IDS_UNTITLED, szCurFile,
					   iPathNameFormat, bModified || iEncoding != iOriginalEncoding,
					   IDS_READONLY, bReadOnly, szTitleExcerpt);
	...
	if (fSuccess) {
		...
		bModified = FALSE;
		//bReadOnly = FALSE;
		SendEditMsg(SCI_SETEOLMODE, iEOLMode, 0);
