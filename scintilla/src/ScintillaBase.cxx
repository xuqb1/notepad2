...
public:
	...
	void PropSet(char *key, const char *val);
...
void LexState::PropSet(char *key, const char *val) {
	props.Set(key, val);
	if (instance) {
...
sptr_t ScintillaBase::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case SCI_SETPROPERTY:
		//const char *pc = reinterpret_cast<const char *>(wParam);
		if (!reinterpret_cast<const char *>(wParam)) {
			return 0;
		}
		DocumentLexState()->PropSet(reinterpret_cast<char *>(wParam),
		          reinterpret_cast<const char *>(lParam));
		break;
